 ...
 * @author 思之声
 */
@WebServlet(name = "DisplayMessageS", urlPatterns = {"/DisplayMessageS"})
public class DisplayMessageS extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (type != null && !type.equals("")) {
            List<Message> messages = DaoFactory.GetMessageDaoImp().GetMessageByMessageType(type);
            request.setAttribute("messages", messages);

        } else {
            List<Message> messages = DaoFactory.GetMessageDaoImp().GetAllMessage();
            request.setAttribute("messages", messages);
