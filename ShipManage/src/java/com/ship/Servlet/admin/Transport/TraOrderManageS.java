 ...
 * @author 思之声
 */
@WebServlet(name = "TraOrderManageS", urlPatterns = {"/Admin/TraOrderManageS"})
public class TraOrderManageS extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (constructOrderId != null && !"".equals(constructOrderId)) {
            ...
            pagination.setUrl("TraOrderManageS?constructOrderId=" + constructOrderId);
            if (DaoFactory.GetConstructOrderDaoImp().FindConstructOrder(constructOrderId)) {
                ConstructOrder constructOrder = DaoFactory.GetConstructOrderDaoImp().GetAConstructOrderByConstructOrderId(constructOrderId);
                if (constructOrder != null && constructOrder.getConstruct_Company_id().equals(loginManager.getManager_Company_id())) {
                    constructOrders.add(constructOrder);
                }
            }

        } else {
            pagination.setUrl("TraOrderManageS?");
        ...
        request.getRequestDispatcher("/admin/TraOrderManage.jsp").forward(request, response);
